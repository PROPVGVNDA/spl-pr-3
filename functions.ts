...
function addLesson(lesson: Lesson): boolean {
    let conflict: ScheduleConflict | null = validateLesson(lesson)
    if (!conflict) { 
...
function findAvailableClassrooms(timeSlot: TimeSlot, dayOfWeek: DayOfWeek): string[] {
    let occupiedClassroomNumbers: string[] = []
    let classroomNumbers: string[] = getClassroomNumbers(classrooms)
...
function validateLesson(lesson: Lesson): ScheduleConflict | null {
    let professorSchedule: Lesson[] = getProfessorSchedule(lesson.professorId);

    for (let element of professorSchedule) {
...
function getMostPopularCourseType(): CourseType {
    let courseMap = new Map<CourseType, number>([
        ["Lab", 0],
        ["Lecture", 0],
        ["Practice", 0],
        ["Seminar", 0]
...
function reassignClassroom(lesson: Lesson, newClassroomNumber: string): boolean {
    let newLesson: Lesson = lesson;
    newLesson.classroomNumber = newClassroomNumber;
    let conflict: ScheduleConflict | null = validateLesson(newLesson);
...
function cancelLesson(lesson: Lesson): void {
    let index: number = schedule.findIndex(element => element === lesson);
